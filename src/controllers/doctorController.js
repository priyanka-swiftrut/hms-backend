...
class DoctorController {
    ...
    async getPatientRecord(req, res) {
        try {
            const { role, _id: userId, hospitalId } = req.user; // Extract role, userId, and hospitalId
            const { filter, search } = req.query; // `filter` for date range, `search` for patient name
            ...
            const appointments = await Appointment.find(query)
                .populate("patientId", "fullName age gender profilePicture") // Populate patient fields
                .limit(10);
